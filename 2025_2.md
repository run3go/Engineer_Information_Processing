...
1. `struct node`는 정수 변수 `p`와 다음 노드를 가리키는 포인터 `n`을 갖는 구조체이다. (연결 리스트)
2. `a.n = &b; b.n = &c; c.n = NULL`: a -> b, b -> c, c는 `NULL`로 초기화
3. `c.n = &a; a.n = &b; b.n = NULL`: c -> a, a -> b, b는 `NULL`로 초기화
4. `struct node* head = &c`: c 노드를 `head`에 할당
5. `head->p = c.p`는 `3`, `head->n->p = c.n.p = a.p`는 `1`, `head->n->n->p = c.n.n.p = a.n.p = b.p`는 `2`
6. <b>출력</b>: 3 1 2
...
## 17. 다음은 Python언어의 문제이다. 아래 코드를 보고 알맞는 출력값을 작성하시오.
...
1. `lst` 배열을 반복하며 곱하기 2한 값으로 딕셔너리 `dst` 생성 ([1:2, 2:4, 3:6])
2. `dst` 값으로 set 생성 ([2,4,6])
3. `lst[0]`의 값을 99로 변경 ([99,2,3])
4. `dst[2]`의 값을 7로 변경 ([1:2, 2:7, 3:6])
5. `s`에 99를 추가 ([2,4,6,99])
6. `set(dst.values())`는 [2,7,6]이 되고, `s`와의 교집합을 구함 ([2,6])
7. <b>출력</b>: 2
...
1. `struct node` 연결 리스트 형태의 구조체
2. `"BEST"`를 매개변수로 `func` 함수를 호출
3. 비어있는 연결 리스트 포인터 `h`와 `n`을 선언
4. `n = malloc(sizeof(struct node))` 구조체의 길이만큼 메모리를 할당하고, 이를 변수 `n`에 할당
5. `n->c = *s++`: 문자열의 문자를 `n.c`에 할당
6. `n->p = h`: `n` 노드에 `h` 노드를 연결
7. `h = n` 새 노드가 새로운 헤드가 됨
8. 이를 모든 문자에서 반복 (T->S->E->B)
9. `putchar(n->c)`: 문자를 출력
10. `n = n->p` 다음 노드로 이동한 후
11. `free(t)` 이전 노드 메모리 해제
12. 출력: TSEB
...
![20번 문제 사진](image-8.png)

<details>
<summary>정답</summary>

**_1.TTL 2.부장 3.대리 4.과장 5.차장_**
...
<b>π Project</b>: 원하는 데이터를 수직적으로 도출

...
**_CREATE INDEX idx_name ON student(name)_**
...
이 문장은 `student` 테이블 내의 `name` 컬럼에 `idx_name`이라는 이름의 인덱스를 생성한다.

테이블을 지정할 때, DML에서 `FROM`을 사용하는 것과 다르게 `ON`을 사용한다는 사실을 염두할 것
...
**_웹 페이지의 입력값을 통해서 SQL 명령어를 주입하여 오동작을 일으키는 해킹방법_**
...
1. <b>요구사항 분석</b>: 데이터베이스가 어떤 기능을 수행해야 하는지 정의

2. <b>개념적 설계</b>: 실제 사용자 관점에서 데이터의 주요 엔티티와 관계를 추상적으로 표현. 구체적인 데이터베이스 기술에 구애받지 않고 <b>비즈니스 요구사항</b>을 중심으로 모델링

3. <b>논리적 설계</b>: 개념적 모델을 특정 데이터베이스 모델로 구체화한다. 스키마를 정의하고 테이블, 컬럼, 제약 조건 등을 세부적으로 설계

4. <b>물리적 설계</b>: 논리적 설계를 실제 데이터베이스 시스템에 맞게 최적화한다. 데이터의 저장 구조, 인덱스 전략, 성능 향상을 위한 물리적 배치 등 구체적인 구현 세부사항을 포함
...
1. `B` 클래스를 생성 (10)
2. `super(n)`: 부모 클래스의 생성자를 호출 (n=10)
3. `a = n`: 변수 `a`에 `n`을 할당 (a=10)
4. `super.print()`: `A` 클래스의 `print` 메서드를 호출
5. 출력: `a=10`
...
<b>형상 관리</b>: 소프트웨어 개발 과정에서 변화하는 산출물을 관리하기 위한 프로세스이다. 소프트웨어의 각 구성 요소의 버전을 관리하고, 변경 사항을 추적하며, 실수를 최소화하는 것을 목표로 한다.

<b>관련 도구</b>: CVS, SVN, ClearCase, Git, Mercurial

<b>VCS</b>(Version Control System): 형상 관리를 지원하기 위한 도구(수단), 버전 관리 도구
